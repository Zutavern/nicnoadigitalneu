...
async function main() {
  console.log('🚀 Starte Stripe Plans Synchronisation...\n')

  for (const plan of STRIPE_PLANS) {
    console.log(`📦 ${plan.planType} - ${plan.name}...`)
    ...
    console.log(`   ✅ ${plan.name} synchronisiert`)
  }

  console.log('\n🎉 Alle Pläne erfolgreich synchronisiert!')
  
  // Zeige Zusammenfassung
  const count = await prisma.subscriptionPlan.count({ where: { isActive: true } })
  console.log(`\n📊 Gesamt aktive Pläne in DB: ${count}`)
}
