...
export default function PostDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleDuplicate = async () => {
    try {
      ...
      router.push(`/salon/marketing/social-media/posts/${duplicate.id}/edit`)
    } catch (error) {
  ...
  if (!post) {
    return (
      <div className="p-6 text-center">
        ...
        <Button asChild>
          <Link href="/salon/marketing/social-media/posts">Zurück zur Übersicht</Link>
        </Button>
      </div>
    )
  }
        ...
        <div className="flex items-center gap-2">
          {canEdit && (
            <Button variant="outline" asChild>
              <Link href={`/salon/marketing/social-media/posts/${post.id}/edit`}>
