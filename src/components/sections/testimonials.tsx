...
export function Testimonials() {
  ...
  const fetchTestimonials = async () => {
    try {
      const [stylistRes, salonRes] = await Promise.all([
        fetch('/api/testimonials?role=STYLIST'),
        fetch('/api/testimonials?role=SALON_OWNER'),
      ])
  ...
  useEffect(() => {
    if (!isLoading && currentTestimonials.length === 0 && hasAnyTestimonials) {
  ...
  }, [isLoading, currentTestimonials.length, hasAnyTestimonials, activeTab, testimonials.STYLIST.length, testimonials.SALON_OWNER.length])
